...
1. [Can I use...](https://caniuse.com/) にアクセス
2. 利用したい機能を入力（`arrow function`, `array.prototype.find`など）
3. 「IE ⚠️」の列の色を確認

「10」が含まれるマスが赤色ならば利用不可能、緑色ならば利用できると考えることができます。

特に、新しい機能の場合には、「IE ⚠️」の列がないこともあります。この場合にも利用不可能と考えることができます。

:::note[補足]
Can I use...は、ブラウザごとのウェブ機能の対応状況を可視化したサイトです。「IE ⚠️」は、昔使用されていたブラウザである「Internet Explorer」のことであり、マス内の数値はそのバージョンを示しています。

Internet ExplorerがES6への対応を開始したのはバージョン11（IE 11）からであるため、それ以前のバージョンであるIE 10などで利用できるかどうかを簡易的な目安にできます。
:::
## スクリプトの文法
