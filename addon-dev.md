...
RTM Wikiでは、もっとも一般的な`.mqo`ファイルを使用したアドオンの制作方法を解説します。
...
3DCG制作ソフトで制作された3Dモデルにマッピングするテクスチャを用意する必要があるため、ペイントソフトも多くの場合必要となります。Windows標準のペイントも使用することはできますが、アルファチャンネルや、レイヤーなどの機能を有していないため、それらの機能を持つペイントソフトを使用することが望ましいです。RealTrainModでは、無料で利用できるGIMPやpaint.netがよく使用されていますが、`.png`形式の画像データが作成できればよいため、Adobe PhotoshopやCLIP STUDIO PAINT、ibisPaintなどのペイントソフトも使用することが可能です。
